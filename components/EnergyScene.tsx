...
function Core({ detail = false }: { detail?: boolean }) {
  ...
  useFrame(({ clock, pointer }, delta) => {
    ...
    group.current.rotation.x += (pointer.y * .22 - group.current.rotation.x) * follow
    group.current.rotation.y += (pointer.x * .35 + t * .09 - group.current.rotation.y) * follow
    group.current.position.y = Math.sin(t * .65) * .1
    core.current.rotation.z = t * .12
  })
  const rings = [[0, 0, 0], [.62, .35, .8], [-.55, -.35, .6]]
  return <group ref={group} rotation={[.04, -.12, 0]}><Float speed={1} rotationIntensity={.12} floatIntensity={.24}>
...
export default function EnergyScene({ detail = false }: { detail?: boolean }) { return <Canvas frameloop="always" dpr={[1, 1.35]} camera={{ position:[0,0,4.4], fov:42 }} gl={{ alpha:true, antialias:true, powerPreference:'high-performance' }}>
